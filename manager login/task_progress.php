...
if(mysqli_query($conn, $query)) {
    ...
    if(isset($_GET['type'])) {
        ...
        $redirectUrl = "task_progress.php?task_id=" . $task_id;
    }
...
$query = "SELECT t.task_id, t.task_title, e.emp_name AS assignee_name
          FROM tasksdetails t
          INNER JOIN employees_login e ON t.assignee = e.emp_id
            ...
            if(isset($_GET['type'])) {
                ...
                switch($taskType) {
                    ...
                    case 'hold':
                        ...
                        $query = "SELECT p.project_name, t.task_id, t.task_title, t.status, t.status_updated_at, t.due_date, t.created_at, e.emp_name as assignee_name, t.comment
                        FROM tasksdetails t
                        INNER JOIN projectsnew p ON t.project_id = p.project_id
                        LEFT JOIN employees_login e ON t.assignee = e.emp_id
                        WHERE p.manager_id = '$managerId' AND t.status = '$taskType'";
                        break;
                    case 'overdue':
                        // Fetch overdue tasks from the database
                        $query = "SELECT p.project_name, t.task_id, t.status, t.task_title, t.status_updated_at, t.due_date, t.created_at, e.emp_name as assignee_name
                                  FROM tasksdetails t
                                  INNER JOIN projectsnew p ON t.project_id = p.project_id
                                  LEFT JOIN employees_login e ON t.assignee = e.emp_id
                                  WHERE p.manager_id = '$managerId' AND t.due_date < NOW() AND t.status != 'completed'";
                        break;
                    default:
                        // Invalid task type
                ...
                // Type parameter not set in the URL
                echo "<tr><td colspan='7'>Task type not specified</td></tr>";
            }
        } else {
            // Session not started or manager_id not set
            echo "<tr><td colspan='7'>Session not started or manager_id not set.</td></tr>";
        }
        ?>
    </table>
...
<div id="messageContainer" class="message-container">
    ...
    <form id="messageForm" action="task_progress.php?type=<?php echo $_GET['type']; ?>" method="post">
...
$(document).ready(function() {
    ...
    $(document).on('click', '.openChatBtn', function() {
        ...
        var assigneeId = $(this).data('assignee-id'); // Get the assignee ID from the data attribute
        $('#chatBox').show();
        loadChatHistory(taskId);
        // Set task ID and assignee in the form
